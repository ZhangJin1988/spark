...
/**
  * 实现了面向stage的调度机制的高层次的调度层 它为每一个job计算一个stage的DAG 有向五环图
  * 追踪RDD和stage的输出是否被物化 （物化就是说，写入了磁盘或者内存地方） 并且寻找一个最小消耗（最优，最小）
  * 调度机制来运行job。它会将stage作为tasksets提交到底层的taskSchedulerImpl上 来在集群上运行它们 task
  *
  * 除了处理stage的DAG 它还负责决定运行每个task的最佳位置，基于当前的缓存状态，将这些最佳位置提交给底层的TaskSchedulerImpl
  * 此外，它会处理由于shuffle输出文件丢失导致的失败，在这种情况下 旧的stage可能就会被重新提交，一个stage内部的失败，如果不是由于shuffle文件
  * 丢失所导致的，会被TaskScheduler处理，它会多次重试每一个Task 直到最后 实在不行了 才会取消整个stage 反复取消就是挂掉了
  */
/**
